...
export class NftProxyContract extends ContractAbstract {
    public static contractName = 'NftProxyContract';
	  ...
	  public async endGrantAuthentication(addr:string,opt?: IOption){
			...
			const _opt = rest
			const gasLimit = await __signer.estimateGas.endGrantAuthentication(addr,_opt)
	  ...
	  public async revokeAuthentication(addr:string,opt?: IOption){
			...
			const __signer = this.getContractSigner(address)
			const _opt = rest
			const gasLimit = await __signer.estimateGas.revokeAuthentication(addr,_opt)
	  ...
	  public async startGrantAuthentication(addr:string,opt?: IOption){
			...
			const __signer = this.getContractSigner(address)
			const _opt = rest
			const gasLimit = await __signer.estimateGas.startGrantAuthentication(addr,_opt)
	  ...
	  public async registerProxy(opt?: IOption){
			...
			const __signer = this.getContractSigner(address)
			const _opt = rest
			const gasLimit = await __signer.estimateGas.registerProxy(_opt)
	  ...
	  public async transferOwnership(_newOwner:string,opt?: IOption){
			...
			const __signer = this.getContractSigner(address)
			const _opt = rest
			const gasLimit = await __signer.estimateGas.transferOwnership(_newOwner,_opt)
	  ...
	  public async grantInitialAuthentication(authAddress:string,opt?: IOption){
			...
			const __signer = this.getContractSigner(address)
			const _opt = rest
			const gasLimit = await __signer.estimateGas.grantInitialAuthentication(authAddress,_opt)
